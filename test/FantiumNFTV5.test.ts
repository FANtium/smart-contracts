...
import { FantiumNFTV5 } from '../typechain-types/contracts/FantiumNFTV5'
...
describe("FantiumNFTV5", () => {

    let nftContract: FantiumNFTV4
    let nftContractV5: FantiumNFTV5
    ...
    beforeEach(async () => {
        ...
        const FantiumNFTV4 = await ethers.getContractFactory("FantiumNFTV4")
        nftContract = await upgrades.deployProxy(FantiumNFTV4, ["FANtium", "FAN", defaultAdmin.address], { initializer: 'initialize' }) as FantiumNFTV4
        ...
        //////////////////////////////////////////////////////////////*/
        // set payment token
        await nftContract.connect(upgraderRole).updatePlatformAddressesConfigs(erc20Contract.address, claimContract.address, userManager.address, forwarder.address)
    ...
    it("Check Setup: upgrading contract and minting NFTs on upgraded contract", async () => {
        const fanV3 = await ethers.getContractFactory("FantiumNFTV5")
        const nftContractV5 = await upgrades.upgradeProxy(nftContract.address, fanV3 ) as FantiumNFTV5
        await nftContractV5.connect(upgraderRole).updatePlatformAddressesConfigs(erc20Contract.address, claimContract.address, userManager.address, forwarder.address)
        await nftContractV5.connect(platformManager).updateCollectionSales(1,maxInvocations,price,tournamentEarningsShare1e7,otherEarningsShare1e7,fantium.address,fantiumSecondaryBPS)

        // check name   
        expect(await nftContractV5.name()).to.equal("FANtium")

        // add platfrom manager to KYC
        await userManager.connect(kycManager).addAddressToKYC(fan.address)
        
        // approve erc20
        await erc20Contract.connect(fan).approve(nftContractV5.address, 3 * price * (10 ** decimals))

        // default admin mint a token
        await nftContractV5.connect(fan).mint(1, 3)

        // check balance
        expect(await nftContractV5.balanceOf(fan.address)).to.equal(3)
    })
    ...
    it("Check Setup: contract upgrade and state preservation", async () => {
        ...
        await userManager.connect(kycManager).addAddressToKYC(fan.address)
        await nftContract.connect(fan).batchMintTo([fan.address],[1],[1]);
        ...
        /// CHECK STATE
        expect(await nftContractV5.name()).to.equal("FANtium")
        expect(await nftContractV5.balanceOf(fan.address)).to.equal(1)
    ...
    it("checks that PLATFORM MANAGER can pause contract", async () => {
        ...
        // pause contract
        await nftContractV5.connect(platformManager).pause()
    ...
    it("checks that FAN cannot mint if NOT kyced", async () => {
        ...
        const fanV3 = await ethers.getContractFactory("FantiumNFTV5")
        const nftContractV5 = await upgrades.upgradeProxy(nftContract.address, fanV3) as FantiumNFTV5
        await nftContractV5.connect(upgraderRole).updatePlatformAddressesConfigs(erc20Contract.address, erc20Contract.address, userManager.address, forwarder.address)

        // check status of fan address
        expect(await userManager.isAddressKYCed(fan.address)).to.equal(false)

        // ty to mint
        await expect(nftContractV5.connect(fan).mint(1,1)).to.be.revertedWith("Address is not KYCed");
    })
    ...
    it("checks that FAN cannot mint if kyced & on allowlist & collection mintable & collection paused & Allowance is too low", async () => {
        ...
        // check if fan can mint
        await expect(nftContractV5.connect(fan).mint(1,1)).to.be.revertedWith("ERC20: insufficient allowance");
    })
    ...
    it("checks that FAN can mint if kyced & on allowlist & collection minting paused & price is correct", async () => {
        ...
        // add fan address to KYC
        await userManager.connect(kycManager).addAddressToKYC(fan.address)
        await nftContractV5.connect(platformManager).toggleCollectionPaused(1)
        ...
        await nftContractV5.connect(fan).mint(1,1);
    ...
    it("checks that FAN cannot mint if kyced & on allowlist & collection minting paused & price is correct & allowlist allocation is used up", async () => {
        ...
        await expect(nftContractV5.connect(fan).mint(1,1)).to.be.revertedWith("Collection is paused or allowlist allocation insufficient");
    })
    ...
    it("minting checks", async () => {
        ...
        await expect(nftContractV5.connect(fan).mint(10,1)).to.be.revertedWith("Collection does not exist");

        await nftContractV5.connect(platformManager).toggleCollectionMintable(1)
        await expect(nftContractV5.connect(fan).mint(1,1)).to.be.revertedWith("Collection is not mintable");
        await nftContractV5.connect(platformManager).toggleCollectionMintable(1)
        await nftContractV5.connect(platformManager).updateCollectionLaunchTimestamp(1, 1981692934)
        await expect(nftContractV5.connect(fan).mint(1,1)).to.be.revertedWith("Collection not launched");
        await nftContractV5.connect(platformManager).updateCollectionLaunchTimestamp(1, 1)

        await nftContractV5.connect(platformManager).updateCollectionSales(1,6,1,tournamentEarningsShare1e7,otherEarningsShare1e7,fantium.address,fantiumSecondaryBPS)
        await expect(nftContractV5.connect(fan).mint(1,10)).to.be.revertedWith("Max invocations surpassed with amount");

        await nftContractV5.connect(fan).mintTo(fan.address,1,1)
        expect(await nftContractV5.connect(fan).balanceOf(fan.address)).to.equal(1)
    ...
    it("checks that FAN can batchmint 10 NFTs if collection is Mintable and Unpaused", async () => {
        ...
        await nftContractV5.connect(fan).batchMintTo([fan.address],[1],[10])
        await expect(nftContractV5.connect(fan).batchMintTo([fan.address],[1,1],[10])).to.be.revertedWith("Arrays must be of equal length")

        /// 
        expect(await nftContractV5.balanceOf(fan.address)).to.equal(10)
    })
    ...
    it("checks that FAN can't batchMint if collection is not Mintable", async () => {
        ...
        await expect(nftContractV5.connect(fan).batchMintTo([fan.address],[1],[10])).to.be.revertedWith("Collection is not mintable")
    })
    ...
    it("checks that FAN can mint if collection is Mintable, Paused and has sufficient allowlist allocation", async () => {
        ...
        await nftContractV5.connect(fan).mint(1, 10);
        /// 
        expect(await nftContractV5.balanceOf(fan.address)).to.equal(10)

        // check that tokenIds are minted
        expect(await nftContractV5.tokenURI(1000000)).to.equal("https://contract.com/1000000")
    ...
    it("checks that OTHER cannot add collection if NOT PLATFORM MANAGER", async () => {
        ...
        await expect(nftContractV5.connect(other)
            .addCollection(
                athlete.address,
                primarySalePercentage,
                secondarySalePercentage,
    ...
    it("checks that PLATFORM MANAGER cannot add collections with 0x0 athlete address", async () => {
        ...
        await expect(nftContractV5.connect(platformManager)
            .addCollection(
                nullAddress,
                primarySalePercentage,
                secondarySalePercentage,
                ...
                tournamentEarningsShare1e7,
                otherEarningsShare1e7
            )).to.be.revertedWith('FantiumNFTV5: addresses cannot be 0')
    })
    ...
    it("checks that PLATFORM MANAGER cannot add faulty collection parameters", async () => {
        ...
        await expect(nftContractV5.connect(platformManager)
            .addCollection(
                ...
                tournamentEarningsShare1e7,
                otherEarningsShare1e7
            )).to.be.revertedWith('FantiumNFTV5: athletePrimarySalesBPS must be less than 10,000')
        ...
        await expect(nftContractV5.connect(platformManager)
            .addCollection(
                ...
                tournamentEarningsShare1e7,
                otherEarningsShare1e7
            )).to.be.revertedWith('FantiumNFTV5: secondary sales BPS must be less than 10,000')
        ...
        await expect(nftContractV5.connect(platformManager)
            .addCollection(
                ...
                tournamentEarningsShare1e7,
                otherEarningsShare1e7
            )).to.be.revertedWith('FantiumNFTV5: max invocations must be less than 10,000')
        ...
        await expect(nftContractV5.connect(platformManager)
            .addCollection(
                ...
                1e7+1,
                otherEarningsShare1e7
            )).to.be.revertedWith('FantiumNFTV5: token share must be smaller 1e7')
    })
    ...
    it("checks that PLATFORM MANAGER cannot update athlete address with 0x0 address", async () => {
        ...
        await expect(nftContract.connect(platformManager).updateCollectionAthleteAddress(1, ethers.constants.AddressZero)).to.be.revertedWith("FantiumNFTV5: address cannot be 0");
    })
    ...
    it("checks that PLATFORM MANAGER can toggle collection isMintingPause", async () => {
        ...
        expect((await nftContractV5.collections(1)).isPaused).to.equal(false)

        // toggle collection is paused
        await nftContractV5.connect(platformManager).toggleCollectionPaused(1)
        // check collection is paused
        expect((await nftContractV5.collections(1)).isPaused).to.equal(true)
    })
    ...
    it("checks that PLATFORM MANAGER can toggle isActivated", async () => {
        ...
        await nftContractV5.connect(upgraderRole).updatePlatformAddressesConfigs(erc20Contract.address, erc20Contract.address, userManager.address, forwarder.address)
        // check collection is paused
        expect((await nftContractV5.collections(1)).isMintable).to.equal(true)

        // update collection pause status
        await nftContractV5.connect(platformManager).toggleCollectionMintable(1)

        // check collection is unPaused
        expect((await nftContractV5.collections(1)).isMintable).to.equal(false)
    })
    ...
    it("checks that PLATFORM MANAGER can update collection athlete address", async () => {
        ...
        expect((await nftContractV5.collections(1)).athleteAddress).to.equal(athlete.address)

        // update collection athlete address
        await nftContractV5.connect(platformManager).updateCollectionAthleteAddress(1, other.address)

        // check collection athlete address
        expect((await nftContractV5.collections(1)).athleteAddress).to.equal(other.address)
    })
    ...
    it("checks that PLATFROM MANAGER can update athlete primary market royalty BPS", async () => {
        ...
        expect((await nftContractV5.collections(1)).athletePrimarySalesBPS).to.equal(9000)

        // update athlete primary market royalty percentage
        await nftContractV5.connect(platformManager).updateCollectionAthletePrimaryMarketRoyaltyBPS(1, 5000)

        // check athlete primary market royalty percentage
        expect((await nftContractV5.collections(1)).athletePrimarySalesBPS).to.equal(5000)

        await expect(nftContractV5.connect(platformManager).updateCollectionAthletePrimaryMarketRoyaltyBPS(1, 10001)).to.be.revertedWith("Max of 100%");
    ...
    it("checks that PLATFROM MANAGER can update athlete secondary market royalty BPS", async () => {
        ...
        expect((await nftContractV5.collections(1)).athleteSecondarySalesBPS).to.equal(500)

        // update athlete secondary market royalty percentage
        await nftContractV5.connect(platformManager).updateCollectionAthleteSecondaryMarketRoyaltyBPS(1, 10)

        // check athlete secondary market royalty percentage
        expect((await nftContractV5.collections(1)).athleteSecondarySalesBPS).to.equal(10)

        await expect(nftContractV5.connect(platformManager).updateCollectionAthleteSecondaryMarketRoyaltyBPS(1, 10001)).to.be.revertedWith("FantiumClaimingV1: secondary sales BPS must be less than 10,000");
    })
    ...
    it("checks that PLATFROM MANAGER can collection Sales", async () => {
        ...
        await userManager.connect(platformManager).addAddressToKYC(fan.address)
        await erc20Contract.connect(fan).approve(nftContractV5.address, price*1000000000)
        await nftContractV5.connect(fan).mint(1,10)
    
        // update platform secondary market address
        await nftContractV5.connect(platformManager).updateCollectionSales(1, 100,1,1,1,other.address,100)
        expect((await nftContractV5.collections(1)).fantiumSalesAddress).to.equal(other.address)
        expect((await nftContractV5.collections(1)).price).to.equal(1)
        
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 100,0,1,1,other.address,100)).to.be.revertedWith("FantiumNFTV5: all parameters must be greater than 0");
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 100,1,0,0,other.address,100)).to.be.revertedWith("FantiumNFTV5: all parameters must be greater than 0");
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 10001,1,1,1,other.address,100)).to.be.revertedWith("FantiumNFTV5: max invocations must be less than 10,000");
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 100,1,1,1,other.address,10001)).to.be.revertedWith("FantiumNFTV5: secondary sales BPS must be less than 10,000");
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 100,1,1,1,nullAddress,100)).to.be.revertedWith("FantiumNFTV5: address cannot be 0");
        await expect(nftContractV5.connect(platformManager).updateCollectionSales(1, 9,1,1,1,other.address,100)).to.be.revertedWith("FantiumNFTV5: max invocations must be greater than current invocations");
    ...
    it("checks that PLATFROM MANAGER can update launch timestmap", async () => {
        ...
        // update platform secondary market address
        await nftContractV5.connect(platformManager).updateCollectionLaunchTimestamp(1, closeTime)
        expect((await nftContractV5.collections(1)).launchTimestamp).to.equal(closeTime)
    ...
    it("checks that PLATFROM MANAGER can update baseURI", async () => {
        ...
        // update platform secondary market address
        await nftContractV5.connect(platformManager).updateBaseURI("https://fantium.io/")
        expect((await nftContractV5.baseURI())).to.equal("https://fantium.io/")
    ...
    it("checks that PLATFORM MANAGER can update platform secondary market address", async () => {
        ...
        expect((await nftContractV5.collections(1)).fantiumSalesAddress).to.equal(fantium.address)
        ...
        expect((await nftContractV5.collections(1)).fantiumSecondarySalesBPS).to.equal(100)
    })
    ...
    it("checks that PLATFORM MANAGER can mint even if collection is not launched", async () => {
        ...
        // approve nft contract to spend Mock20
        await erc20Contract.connect(platformManager).approve(nftContractV5.address, price * 10 ** decimals)

        // mint NFT
        await userManager.connect(platformManager).addAddressToKYC(platformManager.address)
        await nftContractV5.connect(platformManager).mint(1,1);

        // check NFT owner
        expect(await nftContractV5.ownerOf(1000000)).to.equal(platformManager.address)
    })
    ...
    it("check token upgrade can't be triggered by unauthorized", async () => {
        ...
        await expect(nftContractV5.connect(platformManager).upgradeTokenVersion(1000000)).to.revertedWith("Invalid tokenId")

        await erc20Contract.connect(fan).approve(nftContract.address, price * 10 ** decimals * 10)
        await nftContractV5.connect(fan).mint(1, 10);

        // upgrade token without setting claim contract
        await expect(nftContractV5.connect(platformManager).upgradeTokenVersion(1000000)).to.revertedWith("Only claim contract can call this function")
    ...
    it("checks getter function", async () => {
        ...
        expect( await nftContractV5.connect(platformManager).getCollectionAthleteAddress(1)).to.equal(athlete.address)
        expect(await nftContractV5.connect(platformManager).getEarningsShares1e7(1))
        expect(await nftContractV5.connect(platformManager).getCollectionExists(1)).to.equal(true)
        expect(await nftContractV5.connect(platformManager).getMintedTokensOfCollection(1)).to.equal(0)
    })
    ...
    it("using OS filte functions and seeing normal functionality", async () => {
        ...
        /// approvals
        await nftContractV5.connect(fan).approve(fantium.address, 1000001)
        expect(await nftContractV5.getApproved(1000001)).to.be.equal(fantium.address)
        await nftContractV5.connect(fan).setApprovalForAll(fantium.address, true)
        expect(await nftContractV5.isApprovedForAll(fan.address,fantium.address)).to.be.equal(true)
        expect(await nftContractV5.isApprovedForAll(fan.address,claimContract.address)).to.be.equal(false)

        //transfers
        expect(await nftContractV5.connect(fan).balanceOf(fan.address)).to.equal(3)
        await nftContractV5.connect(fantium).transferFrom(fan.address,fantium.address, 1000001)
        expect(await nftContractV5.connect(fan).balanceOf(fan.address)).to.equal(2)
    ...
    it("check trusted forwarder", async () => {
        ...
        expect(await nftContractV5.connect(platformManager).isTrustedForwarder(forwarder.address)).to.equal(true)
        expect(await nftContractV5.connect(platformManager).isTrustedForwarder(fan.address)).to.equal(false)

    })

    // TRUSTED FORWARDER 
    it("check batchTransfer", async () => {
        /// DEPLOY V3
        const fanV3 = await ethers.getContractFactory("FantiumNFTV5")
        const nftContractV5 = await upgrades.upgradeProxy(nftContract.address, fanV3) as FantiumNFTV5
        ...
        await nftContractV5.connect(fan).batchTransferFrom(fan.address, other.address, [1000001,1000002,1000003]) 

        expect(await nftContractV5.connect(fan).ownerOf(1000001)).to.equal(other.address)
        expect(await nftContractV5.connect(fan).ownerOf(1000002)).to.equal(other.address)
        expect(await nftContractV5.connect(fan).ownerOf(1000003)).to.equal(other.address)
        expect(await nftContractV5.connect(fan).ownerOf(1000004)).to.equal(fan.address)
        expect(await nftContractV5.connect(fan).ownerOf(1000005)).to.equal(fan.address)
        expect(await nftContractV5.connect(fan).ownerOf(1000006)).to.equal(fan.address)
