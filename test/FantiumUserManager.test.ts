...
describe("FantiumUserManager", () => {
    ...
    it("Check: setup and contract allowing", async () => {
        ...
        await expect(userManager.connect(platformManager).addAllowedConctract(nullAddress)).to.revertedWith("No null address allowed")
        await expect(userManager.connect(platformManager).removeAllowedConctract(nullAddress)).to.revertedWith("No null address allowed")
    })
    ...
    it("Check: add to KYC and remove", async () => {
        ...
        await userManager.connect(platformManager).addBatchtoKYC([fan.address, fantium.address, athlete.address, other.address])
    ...
    it("Check: add to and remove from IDENT", async () => {
        ...
        await userManager.connect(platformManager).addBatchtoIDENT([fan.address, fantium.address, athlete.address, other.address])
    ...
    it("Check: add to and remove from allowlist", async () => {
        ...
        await userManager.connect(platformManager).batchAllowlist([1],nftContract.address,[fan.address, fantium.address, athlete.address, other.address],[1,1,1,1])
    ...
    it("Check: only allowed contracts and existing collections", async () => {
        ...
        await expect(userManager.connect(platformManager).batchAllowlist([1],fan.address,[fan.address],[1])).to.be.revertedWith("Only allowed Contract")
        await expect(userManager.connect(platformManager).reduceAllowListAllocation(1,nftContract.address,fan.address,1))
        await expect(userManager.connect(platformManager).reduceAllowListAllocation(1,fan.address,fan.address,1)).to.be.revertedWith("Only allowed Contract") 

        await expect(userManager.connect(platformManager).batchAllowlist([10],nftContract.address,[fan.address],[1])).to.be.revertedWith("Collection does not exist")
        await expect(userManager.connect(platformManager).reduceAllowListAllocation([10],nftContract.address,fan.address,1)).to.be.revertedWith("Collection does not exist")
    ...
    it("Check: pause/unpuase", async () => {
        ...
        await expect(userManager.connect(fan).batchAllowlist([1],nftContract.address,[fan.address],[1])).to.be.revertedWith("Pausable: paused")
