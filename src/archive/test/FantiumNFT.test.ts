...
describe("FANtiumNFT V1", () => {
    ...
    beforeEach(async () => {
        const [_defaultAdmin, _platformManager, _kycManager, _fantium, _athlete, _fan, _other] =
        ...
        nftContract = (await upgrades.deployProxy(FantiumNFT, ["FANtium", "FAN", defaultAdmin.address], {
    ...
    it("checks that OTHER cannot add collection if NOT PLATFORM MANAGER", async () => {
        await expect(
            nftContract
                .connect(other)
                .addCollection(athlete.address, 90, 5, 100, 10000, 10, timestamp, fantium.address, 250),
        ).to.be.revertedWith(
            "AccessControl: account 0x976ea74026e726554db657fa54763abd0c3a0aa9 is missing role 0xab538675bf961a344c31ab0f84b867b850736e871cc7bf3055ce65100abe02ea",
        );
    });
    ...
    it("checks that PLATFORM MANAGER cannot add collections with 0x0 athlete address", async () => {
        await expect(
            nftContract
                .connect(platformManager)
                .addCollection(ethers.constants.AddressZero, 90, 5, 100, 10000, 10, timestamp, fantium.address, 250),
