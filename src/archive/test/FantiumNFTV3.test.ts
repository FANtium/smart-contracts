...
describe("FantiumNFTV3", () => {
    ...
    it("checks contract parameters", async () => {
        ...
        expect(await nftContract.hasRole(await nftContract.PLATFORM_MANAGER_ROLE(), platformManager.address)).to.equal(
            true,
        );
    ...
    it("checks that FAN can mint if kyced & on allowlist & collection minting paused & price is correct", async () => {
        ...
        await erc20Contract
            .connect(fan)
            .approve(nftContract.address, ethers.utils.parseUnits(price.toString(), await erc20Contract.decimals()));
    ...
    it("checks that FAN cannot mint if kyced & on allowlist & collection minting paused & price is correct & allowlist allocation is used up", async () => {
        ...
        await erc20Contract
            ...
            .approve(
                nftContract.address,
                ethers.utils.parseUnits(price.toString(), await erc20Contract.decimals()).mul(2),
            );
        ...
        await expect(nftContractV3.connect(fan).mint(1, 1)).to.be.revertedWith(
    ...
    it("minting checks", async () => {
        ...
        await expect(nftContractV3.connect(fan).mint(1, 10)).to.be.revertedWith(
    ...
    it("checks that FAN can batchmint 10 NFTs if collection is Mintable and Unpaused", async () => {
        ...
        await expect(nftContractV3.connect(fan).batchMintTo([fan.address], [1, 1], [10])).to.be.revertedWith(
    ...
    it("checks that FAN can't batchMint if collection is not Mintable", async () => {
        ...
        await expect(nftContractV3.connect(fan).batchMintTo([fan.address], [1], [10])).to.be.revertedWith(
            "Collection is not mintable",
        );
    });
    ...
    it("checks that PLATFORM MANAGER cannot update athlete address with 0x0 address", async () => {
        ...
        await expect(
            nftContract.connect(platformManager).updateCollectionAthleteAddress(1, ethers.constants.AddressZero),
        ).to.be.revertedWith("FantiumNFTV3: address cannot be 0");
    });
    ...
    it("checks that PLATFROM MANAGER can update athlete primary market royalty BPS", async () => {
        ...
        await expect(
            nftContractV3.connect(platformManager).updateCollectionAthletePrimaryMarketRoyaltyBPS(1, 10001),
        ).to.be.revertedWith("Max of 100%");
    });
    ...
    it("checks that PLATFROM MANAGER can update athlete secondary market royalty BPS", async () => {
        ...
        await expect(
            nftContractV3.connect(platformManager).updateCollectionAthleteSecondaryMarketRoyaltyBPS(1, 10001),
        ).to.be.revertedWith("FantiumClaimingV1: secondary sales BPS must be less than 10,000");
    });
    ...
    it("checks that PLATFROM MANAGER can collection Sales", async () => {
        ...
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 100, 0, 1, 1, other.address, 100),
        ).to.be.revertedWith("FantiumNFTV3: all parameters must be greater than 0");
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 100, 1, 0, 0, other.address, 100),
        ).to.be.revertedWith("FantiumNFTV3: all parameters must be greater than 0");
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 10001, 1, 1, 1, other.address, 100),
        ).to.be.revertedWith("FantiumNFTV3: max invocations must be less than 10,000");
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 100, 1, 1, 1, other.address, 10001),
        ).to.be.revertedWith("FantiumNFTV3: secondary sales BPS must be less than 10,000");
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 100, 1, 1, 1, nullAddress, 100),
        ).to.be.revertedWith("FantiumNFTV3: address cannot be 0");
        await expect(
            nftContractV3.connect(platformManager).updateCollectionSales(1, 9, 1, 1, 1, other.address, 100),
        ).to.be.revertedWith("FantiumNFTV3: max invocations must be greater than current invocations");
    });
    ...
    it("check token upgrade can't be triggered by unauthorized", async () => {
        ...
        await expect(nftContractV3.connect(platformManager).upgradeTokenVersion(1000000)).to.revertedWith(
            "Invalid tokenId",
        );
