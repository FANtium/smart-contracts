...
describe("FantiumClaim", () => {
    ...
    it("Check Setup: check distributionEvent creation", async () => {
        ...
        expect((await claimContract.distributionEvents(1)).tournamentDistributionAmount).to.equal(
            (tournamentEarnings * 1 * tournamentEarningsShare1e7) / 1e7,
        );
        expect((await claimContract.distributionEvents(1)).otherDistributionAmount).to.equal(
            (otherEarnings * 1 * otherEarningsShare1e7) / 1e7,
        );
        ...
        expect((await claimContract.distributionEventToCollectionInfo(1, 1)).tokenTournamentClaim).to.equal(
            (tournamentEarnings * tournamentEarningsShare1e7) / 1e7,
        );
        expect((await claimContract.distributionEventToCollectionInfo(1, 1)).tokenOtherClaim).to.equal(
            (otherEarnings * otherEarningsShare1e7) / 1e7,
        );
        expect((await claimContract.distributionEventToCollectionInfo(1, 1)).tokenOtherClaim).to.equal(
            (otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    ...
    it("Check Snapshot: check snapshot mechanic", async () => {
        ...
        await expect(claimContract.connect(athlete).takeClaimingSnapshot(1)).to.be.revertedWith(
            "only platform manager",
        );
        await expect(claimContract.connect(platformManager).takeClaimingSnapshot(5)).to.be.revertedWith(
        ...
        expect((await claimContract.getDistributionEvent(1)).tournamentDistributionAmount).to.equal(
            (2 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7,
        );
        expect((await claimContract.getDistributionEvent(1)).otherDistributionAmount).to.equal(
            (2 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    });
    ...
    it("Check Payin: check payin mechanic", async () => {
        ...
        await expect(claimContract.connect(platformManager).addDistributionAmount(10)).to.be.revertedWith(
        ...
        expect((await claimContract.getDistributionEvent(1)).amountPaidIn).to.equal(
            (2 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7 +
                (2 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
        expect(await erc20Contract.balanceOf(claimContract.address)).to.equal(
            (2 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7 +
                (2 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );

        await erc20Contract.connect(athlete).approve(claimContract.address, totalAmount);
        await expect(claimContract.connect(athlete).addDistributionAmount(1)).to.be.revertedWith(
    ...
    it("Check Payin: check batch payin mechanic", async () => {
        ...
        // try distributionEvent that doesn't exist
        await erc20Contract.connect(athlete).approve(claimContract.address, totalAmount);
        await expect(claimContract.connect(athlete).batchAddDistributionAmount([20])).to.be.revertedWith(
            "only athlete",
        );
        ...
        expect((await claimContract.getDistributionEvent(2)).amountPaidIn).to.equal(
            (2 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7 +
                (2 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
        expect(await erc20Contract.balanceOf(claimContract.address)).to.equal(
            (4 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7 +
                (4 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    ...
    it("Check Closing: closing a distributionEvent and checking require statements", async () => {
        ...
        await expect(claimContract.connect(platformManager).closeDistribution(1)).to.be.revertedWith(
        ...
        await expect(claimContract.connect(platformManager).closeDistribution(5)).to.be.revertedWith(
            "Invalid distribution event",
        );
    ...
    it("Check Update: total earnings update", async () => {
        ...
        await expect(
            claimContract.connect(platformManager).updateDistribtionTotalEarningsAmounts(1, 0, 0),
        ).to.be.revertedWith("FantiumClaimingV1: payout already started");
        await expect(
            claimContract.connect(platformManager).updateDistribtionTotalEarningsAmounts(5, 1, 1),
        ).to.be.revertedWith("Invalid distribution event");
        ...
        expect(await erc20Contract.balanceOf(claimContract.address)).to.equal(
            (tournamentEarnings * tournamentEarningsShare1e7) / 1e7 + (otherEarnings * otherEarningsShare1e7) / 1e7,
        );
        ...
        await expect(
            claimContract
                .connect(platformManager)
                .updateDistribtionTotalEarningsAmounts(1, tournamentEarnings / 2, otherEarnings / 2),
        ).to.be.revertedWith("FantiumClaimingV1: total payout amount must be greater than amount already paid in");

        // check new balance
        await claimContract
            .connect(platformManager)
            .updateDistribtionTotalEarningsAmounts(1, 2 * tournamentEarnings, 2 * otherEarnings);
        expect(await erc20Contract.balanceOf(claimContract.address)).to.equal(
            (tournamentEarnings * tournamentEarningsShare1e7) / 1e7 + (otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    });
    ...
    it("Check Update: update collectionId in distributionEvent when already paid in ", async () => {
        ...
        await claimContract.connect(platformManager).updateDistributionEventTimeStamps(1, 1691841787, 1791841787);
        await expect(
            claimContract.connect(platformManager).updateDistributionEventCollectionIds(1, [10, 11, 12]),
        ).to.be.revertedWith("FantiumClaimingV1: collection does not exist");
        ...
        await expect(
            claimContract.connect(platformManager).updateDistributionEventCollectionIds(1, [4, 5, 6]),
        ).to.be.revertedWith("FantiumClaimingV1: total payout amount must be greater than amount already paid in");
    });
    ...
    it("Check Update: distributionEvent Addresses", async () => {
        ...
        await expect(
            claimContract.connect(platformManager).updateDistributionEventAddresses(1, nullAddress, nullAddress),
        ).to.be.revertedWith("FantiumClaimingV1: athlete address cannot be 0");
    });
    ...
    it("Check Update: distributionEvent timestamp", async () => {
        ...
        await expect(
            claimContract.connect(platformManager).updateDistributionEventTimeStamps(1, 0, closeTime + 1),
        ).to.be.revertedWith(
            "FantiumClaimingV1: times must be greater than 0 and close time must be greater than start time and in the future",
        );
        await expect(
            claimContract.connect(platformManager).updateDistributionEventTimeStamps(1, startTime, 0),
        ).to.be.revertedWith(
            "FantiumClaimingV1: times must be greater than 0 and close time must be greater than start time and in the future",
        );
        await expect(
            claimContract.connect(platformManager).updateDistributionEventTimeStamps(1, startTime, startTime),
        ).to.be.revertedWith(
            "FantiumClaimingV1: times must be greater than 0 and close time must be greater than start time and in the future",
        );
        await expect(
            claimContract.connect(platformManager).updateDistributionEventTimeStamps(1, startTime, startTime + 5),
        ).to.be.revertedWith(
            "FantiumClaimingV1: times must be greater than 0 and close time must be greater than start time and in the future",
        );
    });
    ...
    it("Check Update: distributionEvent Fee", async () => {
        ...
        await expect(claimContract.connect(platformManager).updateDistributionEventFee(1, 10001)).to.be.revertedWith(
    ...
    it("Check single claiming: claim, receive amount and check calculation", async () => {
        ...
        await expect(claimContract.connect(other).claim(1000001, 1)).to.be.revertedWith(
        ...
        expect((await claimContract.getDistributionEvent(1)).claimedAmount).to.equal(
            (2 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7 +
                (2 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    ...
    it("Check batch claiming: claim, receive amount and check calculation", async () => {
        ...
        expect((await claimContract.getDistributionEvent(1)).tournamentDistributionAmount).to.equal(
            (11 * tournamentEarnings * tournamentEarningsShare1e7) / 1e7,
        );
        expect((await claimContract.getDistributionEvent(1)).otherDistributionAmount).to.equal(
            (11 * otherEarnings * otherEarningsShare1e7) / 1e7,
        );
    ...
    it("Check Error: try to claim without token and fail", async () => {
        ...
        await expect(claimContract.connect(fan).claim(1000000, 1)).to.be.revertedWith(
        ...
        await erc20Contract.connect(athlete).approve(claimContract.address, totalAmount);
        await claimContract.connect(athlete).addDistributionAmount(1);
        await expect(claimContract.connect(fan).claim(1000001, 1)).to.be.revertedWith(
    ...
    it("Check batch claiming: can't claim with tokens not in snapshot", async () => {
        ...
        // new tokens
        await expect(claimContract.connect(other).claim(1000006, 1)).to.be.revertedWith(
            "FantiumClaimingV1: token not allowed",
        );
        await expect(
            claimContract.connect(other).batchClaim([1000006, 1000007, 1000008, 1000009, 1000010], [1, 1, 1, 1, 1]),
        ).to.be.revertedWith("FantiumClaimingV1: token not allowed");
    ...
    it("Check claiming: try to claim without having paid in", async () => {
        ...
        // new tokens
        await expect(claimContract.connect(other).claim(1000005, 1)).to.be.revertedWith(
            "FantiumClaimingV1: total distribution amount has not been paid in",
        );
    });
    ...
    it("Check claiming: revert wrong length of batch claim", async () => {
        ...
        await expect(claimContract.connect(other).batchClaim([1000001, 1000002, 1000002], [1])).to.be.revertedWith(
        ...
        await expect(claimContract.connect(other).batchClaim(tokenNumbers, events)).to.be.revertedWith(
    ...
    it("Check claiming: check token allowed logic", async () => {
        ...
        await expect(claimContract.connect(other).claim(4000000, 1)).to.be.revertedWith(
            "FantiumClaimingV1: token not allowed",
        );
        await claimContract.connect(other).claim(1000001, 1);
        await expect(claimContract.connect(other).claim(1010001, 1)).to.be.revertedWith(
            "FantiumClaimingV1: token not allowed",
        );
    });
    ...
    it("Check Update: Fantium NFT Contract", async () => {
        ...
        await expect(claimContract.connect(platformManager).updateFantiumNFTContract(nullAddress)).to.be.revertedWith(
    ...
    it("Check Update: Payout Token Contract", async () => {
        ...
        await expect(claimContract.connect(platformManager).updateGlobalPayoutToken(nullAddress)).to.be.revertedWith(
            "Null address not allowed",
        );
    });
    ...
    it("Check Update: Fantium user Manager", async () => {
        ...
        await expect(claimContract.connect(platformManager).updateFantiumUserManager(nullAddress)).to.be.revertedWith(
            "Null address not allowed",
        );
    });
    ...
    it("Check Upgrade: Fantium NFT Contract fails upgrade", async () => {
        ...
        await nftContractV3
            .connect(platformManager)
            .updatePlatformAddressesConfigs(erc20Contract.address, fan.address, userManager.address, forwarder.address);
