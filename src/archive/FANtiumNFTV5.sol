...
// src/archive/contracts/interfaces/IFantiumNFT.sol
...
// src/archive/contracts/interfaces/IFantiumUserManager.sol
...
// src/archive/contracts/utils/TokenVersionUtil.sol
...
// src/archive/contracts/FantiumNFTV5.sol
    ...
    IFantiumNFT
{
    ...
    modifier onlyAthlete(uint256 _collectionId) {
        require(
            _msgSender() == collections[_collectionId].athleteAddress || hasRole(PLATFORM_MANAGER_ROLE, msg.sender),
            "Only athlete"
        );
        _;
    }
     ...
     * @param _amount Amount of tokens to mint.
     */
    function mintTo(address _to, uint256 _collectionId, uint24 _amount) public whenNotPaused {
        // CHECKS
        ...
        uint256 totalPrice = collection.price * 10 ** ERC20Upgradeable(erc20PaymentToken).decimals() * _amount;
        ...
        require(collection.invocations + _amount <= collection.maxInvocations, "Max invocations surpassed with amount");
        ...
        onlyPlatformManager
    {
        require(
            ...
            "FantiumNFTV5: secondary sales BPS must be less than 10,000"
        );
        require(_maxInvocations < 10_000, "FantiumNFTV5: max invocations must be less than 10,000");
        require(_athletePrimarySalesBPS <= 10_000, "FantiumNFTV5: athletePrimarySalesBPS must be less than 10,000");
        require(nextCollectionId < 1_000_000, "FantiumNFTV5: max collections reached");
        require(
            _tournamentEarningShare1e7 <= 1e7 && _otherEarningShare1e7 <= 1e7,
            "FantiumNFTV5: token share must be smaller 1e7"
        );

        require(
            _athleteAddress != address(0) && _fantiumSalesAddress != address(0), "FantiumNFTV5: addresses cannot be 0"
        );
    ...
    function updateCollectionAthleteAddress(
        ...
        onlyPlatformManager
    {
        require(_athleteAddress != address(0), "FantiumNFTV5: address cannot be 0");
        ...
        onlyPlatformManager
    {
        ...
        require(
            ...
            "FantiumNFTV5: all parameters must be greater than 0"
        );
        require(_maxInvocations < 10_000, "FantiumNFTV5: max invocations must be less than 10,000");
        require(nextCollectionId < 1_000_000, "FantiumNFTV5: max collections reached");
        require(
            _tournamentEarningShare1e7 <= 1e7 && _otherEarningShare1e7 <= 1e7,
            "FantiumNFTV5: token share must be less than 1e7"
        );

        require(
            _maxInvocations >= collections[_collectionId].invocations,
            "FantiumNFTV5: max invocations must be greater than current invocations"
        );

        require(_fantiumSalesAddress != address(0), "FantiumNFTV5: address cannot be 0");

        require(
            collections[_collectionId].athleteSecondarySalesBPS + _fantiumSecondarySalesBPS <= 10_000,
            "FantiumNFTV5: secondary sales BPS must be less than 10,000"
        );
        ...
        onlyUpgrader
    {
        require(
            ...
            "FantiumNFTV5: addresses cannot be 0"
        );
