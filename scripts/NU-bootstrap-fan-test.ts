...
import { FantiumNFTV4_Test } from '../typechain-types';
...
async function main() {
    ...
    const contents = readFileSync(join(__dirname, './addresses/Test_fantiumNFT.json'), 'utf-8');
    console.log(JSON.parse(contents));
    const contractAddresses = JSON.parse(contents)

    const nftContract = await ethers.getContractAt("FantiumNFTV4_Test", contractAddresses.proxy, owner) as FantiumNFTV4_Test
