...
async function main() {
  ...
  const FantiumNFT = await ethers.getContractFactory("FantiumNFTV5_Test");
  const fanContract = await upgrades.deployProxy(FantiumNFT, ["FantiumNFTV5_Test", "FANT", deployer.address], { initializer: 'initialize', kind: 'uups'})
